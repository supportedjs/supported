...
async function processPolicies(projectPaths, setupProjectFn, spinner, today, policies, ignoredDependencies) {
  ...
  let progressLogger = new ProgressLogger(spinner, isMultipleProduct, ignoredDependencies.length);
    ...
    work.push(
      queue.add(async () => {
        ...
        let auditResult = await isInSupportWindow(
          dependenciesToCheck.filter(dep => !ignoredDependencies.includes(dep.name)),
          pkg.name,
          {
            progressLogger,
            policies,
