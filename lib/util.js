...
function checkNodeCompatibility(_nodeVersion) {
  ...
  if (!semver.gtr(nodeVersion, '8.*')) {
    throw new Error(
      chalk`{red Node v${nodeVersion} found, minimum node version required to run this tool is Node v10. Please updated the node version.}`,
    );
  }
}
