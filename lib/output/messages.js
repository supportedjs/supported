...
const LOG_LAST_STEP_PROGRESS = (isInSupportWindow, expiringSoonCount) => {
  if (isInSupportWindow && expiringSoonCount) {
    return chalk`{dim [4/4]} {yellow ⚠} Validating policies`;
  }
  if (isInSupportWindow) {
    return chalk`{dim [4/4]} {green ✓} Validating policies`;
  }
  return chalk`{dim [4/4]} {red ✗} Validating policies`;
};
