...
class ProgressLogger {
  constructor(_packagesCount = 0, _spinner = { prefixText: '', text: '' }) {
    this.spinner = _spinner;
    this.totalPackages = 1 + _packagesCount; // start from `1` as we have node policy tested all the time
    this.processedCount = 0;
    this.semVerLogged = false;
  }

  getLoggerPrefixText(name, isSupported, isWarning) {
  ...
  updateSpinner(name, isSupported, isWarning) {
    if (name) {
      ...
      if (name === 'SemVer' && !this.semVerLogged) {
        ...
        this.spinner.prefixText += this.getLoggerPrefixText(name, isSupported, isWarning);
