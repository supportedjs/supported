...
const { sortLibraries, getLatest, updateSpinner, initSpinnerInfo } = require('../util');

module.exports = async function isInSupportWindow(projectRoot, { spinner }) {
  ...
  initSpinnerInfo(dependenciesToCheck.length, spinner);
  ...
  updateSpinner('SemVer', isSemVerSupported, isSemVerExpiring);

  const nodeInfo = getNodeInfo(pkg);
  let result = isLtsOrLatest(nodeInfo, nodeInfo.version);
  supportChecks.push({
    ...result,
    name: nodeInfo.name,
  });
