...
function getBodyContent(results) {
  ...
  results.forEach(({ name, resolvedVersion, latestVersion, duration, isSupported }) => {
    if (!isSupported) {
      ...
      unSupportedTable.push([
        ...
        chalk`{red ${qtrs} ${getQtrLocale(qtrs)}}`,
      ]);
    } else {
  ...
  let table = '';
  if (unSupportedTable.length) {
    table += chalk`
{red Unsupported Pacakges}
${unSupportedTable.toString()}
    `;
  }
  if (expiringSoonTable.length) {
    table += chalk`
{yellow Expiring soon Pacakges}
${expiringSoonTable.toString()}
    `;
  }
  if (supportedTable.length) {
    table += chalk`
{green Supported Pacakges}
 ...
 * @returns {int} total number of expring and violating packages
 */
